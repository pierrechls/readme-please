var colors = require('colors')

colors.setTheme({
  info: 'gray',
  help: 'cyan',
  warning: 'yellow',
  error: 'red',
  success: 'green'
})

const BACK_LINE = '\n'

function format (text, backLineBefore = true, backLineAfter = true) {
  backLineBefore ? text = BACK_LINE + text : text = text
  backLineAfter ? text = text + BACK_LINE : text = text
  return text
}

module.exports = {
  info: function (text, backLineBefore = true, backLineAfter = true) {
    console.log(format(text, backLineBefore, backLineAfter).info)
  },
  help: function (text, backLineBefore = true, backLineAfter = true) {
    console.log(format(text, backLineBefore, backLineAfter).help)
  },
  warning: function (text, backLineBefore = true, backLineAfter = true) {
    console.log(format(text, backLineBefore, backLineAfter).warning)
  },
  error: function (text, backLineBefore = true, backLineAfter = true) {
    console.log(format(text, backLineBefore, backLineAfter).error)
  },
  success: function (text, backLineBefore = true, backLineAfter = true) {
    console.log(format(text, backLineBefore, backLineAfter).success)
  }
}
